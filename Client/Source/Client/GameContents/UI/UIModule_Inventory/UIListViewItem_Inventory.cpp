...
void UUIListViewItem_Inventory::NativeConstruct()
{
	...
	m_pTxt_Name = Cast<UTextBlock>(GetWidgetFromName(TEXT("Ctxt_Name")));
	m_pTxt_Count = Cast<UTextBlock>(GetWidgetFromName(TEXT("Ctxt_Count")));
}

void UUIListViewItem_Inventory::NativeOnListItemObjectSet(UObject* pUObj)
{
	...
	// 이름 세팅
	m_pTxt_Name->SetText(FText::FromString(pUserItemInfo->GetData()->Name));
