...
void UFXContainer::BeginDestroy()
{
...
const bool& UFXContainer::IsLooping() const
{
	return m_bIsLoop;
}
...
FXManager::~FXManager()
{
	...
	{
		// delete
		if (Managers::IsInit() && IsValid(Managers::GameInst))
			Managers::GameInst->RemoveUObject(pair.Value);
	}
...
AFXActor* FXManager::PlayFX(UFXContainer* fxContainer, const FVector& location, const FQuat& rotation, const FVector& scale)
{
	return PlayFX(fxContainer,nullptr, FName(TEXT("None")), false, location, rotation, scale);
}

AFXActor* FXManager::PlayFX(UFXContainer* fxContainer, const FVector& location, const FQuat& rotation, const FVector& scale, const FOnFXReturn& onReturn)
{
	return PlayFX(fxContainer,nullptr, FName(TEXT("None")), false, location, rotation, scale, onReturn);
}

AFXActor* FXManager::PlayFX(UFXContainer* fxContainer, AActor* parent, const FName& socketName,  const bool& bFollowParent, const FVector& location, const FQuat& rotation, const FVector& scale)
{
	// Container 유효성 체크
	if (fxContainer)
	{
...
AFXActor* FXManager::PlayFX(UFXContainer* fxContainer, AActor* parent, const FName& socketName,  const bool& bFollowParent, const FVector& location, const FQuat& rotation, const FVector& scale, const FOnFXReturn& onReturn)
{
	// Container 유효성 체크
	if (fxContainer)
	{
...
UFXContainer* FXManager::RegistFX(UClass* fxClass, int nDefaultCount)
{
	auto fxName = fxClass->GetName();

	// Create Or AddRefCount Container
	UFXContainer* pResult = nullptr;
	...
	{
		pResult = NewObject<UFXContainer>();
		Managers::GameInst->AddUObject(pResult);
...
void FXManager::ReleaseFX(FString fxName)
{
	if (m_map_FXContainer.Contains(fxName))
	{
		...
		// delete
		if (Managers::IsInit() && IsValid(Managers::GameInst))
			Managers::GameInst->RemoveUObject(fxContainer);
	}
}

void FXManager::ReleaseFX(UFXContainer* fxContainer)
{	
	if (fxContainer == nullptr)
		return;
...
AFXActor* FXManager::GenerateFX(UFXContainer* const fxContainer)
{
	AFXActor* pResult;
