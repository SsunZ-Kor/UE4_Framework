...
#include "Client/Framework/Managers/FXManager/FXActor.h"
#include "Client/Framework/Managers/SFXManager/SFXActor.h"
...
void UActionController::OnOwnerTick(float deltaTime)
{
	...
	// Update Event Runtime
	{
		...
		while (pNode_Event)
		{
			...
			// 끝났다면 eventRuntime 종료
			if (!pEventRuntime->IsSameAsActionEndTime() && pEventRuntime->GetEndTime() <= m_fElapsedTime)
			{
	...
	if (bIsCrrActionLastTick)
	{
		...
		{
			//m_fElapsedTime = FMath::Fmod(m_fElapsedTime, fCrrActionLength);
			while (m_fElapsedTime >= fCrrActionLength)
				m_fElapsedTime -= fCrrActionLength;
