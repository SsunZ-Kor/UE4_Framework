...
template<typename T>
class CLIENT_API ActionEventRuntimeBase : public IActionEventRuntime
{
protected:
	const T* m_pData;
...
public:
	...
	virtual void Init(T& data, AGameCharacter* owner, UActionData*pActionLength);
...
template<typename T>
void ActionEventRuntimeBase<T>::Init(T& data, AGameCharacter* owner, UActionData*pActionLength)
{
	m_pData = &data;
	m_pOwner = owner;
...
template <typename T>
const bool ActionEventRuntimeBase<T>::IsSameAsActionEndTime()
{
	return false;
}
